...
export default function ProjectsCarousel() {
	...
	// compute the current page and maxIndex (page-aligned)
	const maxIndex = Math.floor((projects.length - 1) / cardsPerView) * cardsPerView;
	const pageKey = index; // use index as the key for page-level transition
	const pageProjects = projects.slice(index, index + cardsPerView);

	const pageTransitions = useTransition(pageKey, {
		from: {
			opacity: 0,
			transform:
	...
	const handlePrev = () => {
		if (index > 0) {
			setDirection("left");
			setIndex((prev) => Math.max(prev - cardsPerView, 0));
		}
	};
	...
	const handleNext = () => {
		if (index < maxIndex) {
			setDirection("right");
			setIndex((prev) => Math.min(prev + cardsPerView, maxIndex));
		}
	};
			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="relative overflow-hidden">
					...
					<button
						...
						disabled={index === 0}
						className={`absolute left-0 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full transition ${
							index === 0
								? "bg-[#E0ECFF]/50 dark:bg-[#161B22]/50 cursor-not-allowed"
								: "bg-[#E0ECFF] dark:bg-[#161B22] hover:bg-[#CBD5E0] dark:hover:scale-110"
							}`}
					>
					...
					<button
						...
						className={`absolute right-0 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full transition ${
							index >= maxIndex
								? "bg-[#E0ECFF]/50 dark:bg-[#161B22]/50 cursor-not-allowed"
								: "bg-[#E0ECFF] dark:bg-[#161B22] hover:bg-[#CBD5E0] dark:hover:scale-110"
							}`}
					...
					{/* ----------  Carousel track  ---------- */}
					<div className="relative min-h-[480px] h-auto">
